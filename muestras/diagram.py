...
class Trie(object):
    ...
    def diagram(self, filename, render):
        ...
        diagram.attr('node', shape='Mrecord',fillcolor='green',style='filled')
        diagram.attr('edge', arrowsize='0.5')
        ...
        while not q.empty():
            ...
            for child in node.getChildren():
                ...
                if child.getEnding():
                    diagram.attr('node', shape='diamond', style='filled', fillcolor='#ff31ff')
                    diagram.node(str(i), child.getValue()  )
                    diagram.attr('node', shape='Mrecord', fillcolor='white')
                else:
                    diagram.node(str(i), child.getValue())
